...
!function($) {
	...
	DateRangePicker.prototype = {
		...
		notify: function(label) {
			...
			var arg2 = (this.cleared ? null : this.endDate);
			var arg3 = (this.cleared ? null : label);
			this.cleared = false;
			this.cb(arg1, arg2, arg3);
		},
		move: function() {
			if (this.opens == 'left') {
				this.container.css({
		...
		updateDates: function (options) {
			...
			// custom range then use dates provided other wise use the proper dates
			if(found === null) {
				// make sure dates are valid
				if(typeof options.startDate !== 'object' || !(options.startDate instanceof Date)) {
					console.log('Daterangepicker:  startDate is not a valid date');
					return;
				}
				if(typeof options.endDate !== 'object' || !(options.endDate instanceof Date)) {
					console.log('Daterangepicker:  endDate is not a valid date');
					return;
				}

				options.label = this.locale.customRangeLable;
				this.startDate = options.startDate;
				this.endDate = options.endDate;
				this.container.find('.calendar').show();
			} else {
				this.startDate = found[0];
				this.endDate = found[1];
			}
