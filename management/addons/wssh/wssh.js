...
WSSHClient.prototype._generateEndpoint = function (options) {
	...
	// use a custom domain if it was provided
	if (options.domain !== undefined) {
		endpoint += options.domain;
	} else {
		endpoint += window.location.host;
	}
	endpoint += '/wssh/' + encodeURIComponent(options.hostname) + '/' + encodeURIComponent(options.username);

	if (options.authentication_method == 'password') {
		endpoint += '?password=' + encodeURIComponent(options.password) + '&port=' + encodeURIComponent(options.port);
	...
	if (options.command !== "") {
		endpoint += '&run=' + encodeURIComponent(options.command);
	}
	return endpoint;
};
